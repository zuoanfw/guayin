...
/*function get_send_date(send_date){
    ...
    if (send_date != '') {
        $.ajax({
           ...
           // data: {date: '20170528'},
            dataType: "jsonp",
            url: "http://api.goseek.cn/Tools/holiday?date="+send_date+"&jsoncallback=?",//+tab,
            jsonpCallback: "m1",
...
{
    var upurl ='/index.php?m=Home&c=Uploadify&a=uploadfile&num='+num+'&input='+elementid+'&path='+path+'&func='+callback;
    layer.open({
        type: 2,
        title: '上传文件',
        shadeClose: true,
