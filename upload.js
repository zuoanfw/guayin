$(function() {
	...
	function webuploader(opts){
		...
		uploader.on( 'uploadSuccess', function( file,data) {
            ...
            var ext = file.name.split('.');  //获取文件后缀
            ext = ext[ext.length - 1];
			if(data.state == 'SUCCESS'){
				var sLi = "";
				sLi += '<li class="img '+file.id+'">';
				sLi += '<img src="' + data.url + '" width="100" height="100" onerror="this.src=\'__ROOT__/public/plugins/webuploader/images/"+ext+".png\'">';
				...
				uploader.makeThumb( file, function( error, src ) {
					...
					if ( error ) {
                      //  var ext = file.name.split('.');  //获取文件后缀
                       // ext = ext[ext.length - 1];
						$wrap.text( '没有预览图' );
                       // var imgurl = "<img src='/public/plugins/webuploader/images/"+ext+".png' width='100' height='100'>";
                       // $wrap.html( imgurl );
						return;
					}
		...
		uploader.onError = function( type ) {
            ...
            if (type == "Q_TYPE_DENIED") {
                alert("请上传jpg,png,gif,jpeg,zip,rar,psd格式文件");
            } else if (type == "F_EXCEED_SIZE" || type == "Q_EXCEED_SIZE_LIMIT") {
                alert("文件大小不能超过20M,请联系客服单独上传");
            }else {
	...
	window.Manager = {
		...
		upload : function(opts){
			...
			}else if(type == 'printfile'){

                this.opts.accept = {

                    title : '文件',

                    extensions : 'gif,jpg,jpeg,bmp,png,zip,rar,psd',
